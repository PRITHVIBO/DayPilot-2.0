        ...
        foreach ($chunks as $i => $chunkText) {
            ...
            if (!$existing || (string)$existing['embedding_hash'] !== $hash || $existing['embedding_json'] === null) {
                if ($semantic && !$embeddingJson && trim((string)cfg('ai.api_key')) !== '') {
                    try { $embeddingJson = json_encode(gemini_embedding($chunkText,'RETRIEVAL_DOCUMENT'), JSON_THROW_ON_ERROR); $embeddingModel=(string)cfg('ai.embedding_model','gemini-embedding-001'); $embedded++; }
...
function search_memory(string $uid, string $query, int $limit = 8, bool $semantic = true): array {
    ...
    if ($semantic && trim((string)cfg('ai.api_key')) !== '') {
        try { $queryEmbedding = gemini_embedding($query,'RETRIEVAL_QUERY'); } catch (Throwable $e) { error_log('[DayPilot memory query embedding] '.$e->getMessage()); }
    }
    $scored=[];
    foreach($rows as $row){
