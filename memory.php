...
{
    $pdo = db();
    $a = $pdo->prepare('SELECT COUNT(*) c FROM memory_chunks WHERE user_id=?');
    $a->execute([$uid]);
    $total = (int)$a->fetchColumn();
    $b = $pdo->prepare('SELECT COUNT(*) c FROM memory_chunks WHERE user_id=? AND embedding_json IS NOT NULL AND embedding_json<>""');
    $b->execute([$uid]);
    $embedded = (int)$b->fetchColumn();
    $c = $pdo->prepare('SELECT COUNT(*) c FROM work_logs WHERE user_id=?');
    $c->execute([$uid]);
    $logs = (int)$c->fetchColumn();
    $d = $pdo->prepare('SELECT COUNT(*) c FROM notes WHERE user_id=?');
    $d->execute([$uid]);
    $notes = (int)$d->fetchColumn();
    $e = $pdo->prepare('SELECT COUNT(*) c FROM projects WHERE user_id=? AND status="active"');
    $e->execute([$uid]);
    $projects = (int)$e->fetchColumn();
...
function split_memory_text(string $text, int $size = 1800, int $overlap = 240): array
{
    ...
    if ($text === '') return [];
    $size = max(500, $size);
    $overlap = max(0, min($size - 100, $overlap));
    $len = mb_strlen($text);
...
function gemini_embedding(string $text, string $taskType = 'RETRIEVAL_DOCUMENT'): array
{
    $key = trim((string)cfg('ai.api_key'));
    if ($key === '') throw new RuntimeException('Gemini API key is not configured for embeddings.');
    $model = trim((string)cfg('ai.embedding_model', 'gemini-embedding-001')) ?: 'gemini-embedding-001';
    $body = [
    ...
    $resp = http_json(
        'https://generativelanguage.googleapis.com/v1beta/models/' . rawurlencode($model) . ':embedContent',
        ['Content-Type' => 'application/json', 'Accept' => 'application/json', 'x-goog-api-key' => $key],
        $body,
        45
    );
    $values = $resp['embedding']['values'] ?? [];
    if (!is_array($values) || !$values) throw new RuntimeException('Gemini embedding response did not include values.');
...
function memory_source_rows(string $uid, int $maxSources = 30): array
{
    $pdo = db();
    $sources = [];
    $st = $pdo->prepare('SELECT id,title,content,tags,created_at,updated_at FROM notes WHERE user_id=? ORDER BY updated_at DESC LIMIT ' . max(1, min(100, $maxSources)));
    $st->execute([$uid]);
    ...
    $st = $pdo->prepare('SELECT id,project_id,title,content,kind,created_at,updated_at FROM work_logs WHERE user_id=? ORDER BY created_at DESC LIMIT ' . max(1, min(100, $maxSources)));
    $st->execute([$uid]);
    ...
    $st = $pdo->prepare('SELECT id,review_date,content,generated_by,created_at,updated_at FROM daily_reviews WHERE user_id=? ORDER BY review_date DESC LIMIT ' . max(1, min(60, $maxSources)));
    $st->execute([$uid]);
    ...
    $st = $pdo->prepare('SELECT id,name,description,status,created_at,updated_at FROM projects WHERE user_id=? ORDER BY updated_at DESC LIMIT ' . max(1, min(50, $maxSources)));
    $st->execute([$uid]);
...
function memory_index(string $uid, int $maxSources = 12, bool $semantic = true): array
{
    ...
    foreach ($sources as $src) {
        ...
        foreach ($chunks as $i => $chunkText) {
            ...
            $q->execute([$uid, $src['type'], $src['id'], $i]);
            $existing = $q->fetch() ?: null;
            $embeddingJson = $existing['embedding_json'] ?? null;
            $embeddingModel = $embeddingJson ? (string)cfg('ai.embedding_model', 'gemini-embedding-001') : null;
            ...
            if (!$existing || (string)$existing['embedding_hash'] !== $hash || $existing['embedding_json'] === null) {
                if ($semantic && !$embeddingJson && trim((string)cfg('ai.api_key')) !== '') {
                    try {
                        $embeddingJson = json_encode(gemini_embedding($chunkText, 'RETRIEVAL_DOCUMENT'), JSON_THROW_ON_ERROR);
                        $embeddingModel = (string)cfg('ai.embedding_model', 'gemini-embedding-001');
                        $embedded++;
                    } catch (Throwable $e) {
                        $failed++;
                        error_log('[DayPilot memory embedding] ' . $e->getMessage());
                    }
                }
                $now = now();
                if ($existing) {
...
function lexical_memory_score(string $query, string $content, string $title = ''): float
{
    ...
    if (!$q || $hay === '') return 0;
    $score = 0.0;
    $unique = array_values(array_unique($q));
...
function search_memory(string $uid, string $query, int $limit = 8, bool $semantic = true): array
{
    ...
    if ($semantic && trim((string)cfg('ai.api_key')) !== '') {
        try {
            $queryEmbedding = gemini_embedding($query, 'RETRIEVAL_QUERY');
        } catch (Throwable $e) {
            error_log('[DayPilot memory query embedding] ' . $e->getMessage());
        }
    }
    $scored = [];
    foreach ($rows as $row) {
...
function daily_data_snapshot(string $uid, string $date): array
{
    ...
    $st = $pdo->prepare('SELECT id,title,priority,estimated_minutes,completed_at FROM tasks WHERE user_id=? AND status="done" AND DATE(completed_at)=? ORDER BY completed_at DESC');
    $st->execute([$uid, $date]);
    $done = $st->fetchAll();
    ...
    $due = [];
    foreach ($open as $t) {
        if (!empty($t['due_at']) && substr((string)$t['due_at'], 0, 10) === $date) $due[] = $t;
    }
...
{
    $data = daily_data_snapshot($uid, $date);
    $lines = [];
    $done = $data['completed_today'];
    $logs = $data['work_logs'];
    $due = $data['due_today'];
